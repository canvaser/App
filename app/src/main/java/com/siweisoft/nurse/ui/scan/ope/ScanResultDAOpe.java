...
import com.siweisoft.nurse.ui.check.checkblood.fragment.CheckBloodFrag;
...
public class ScanResultDAOpe extends BaseDAOpe {
    ...
    //{"qrtype":"drug","pno":"住院号","zyh":"住院流水号","advno":"医嘱ID","time":"用药时间"}
    ...
    public void sortResult(FragmentActivity activity, String result) {
        ...
        //病人抽血核对
        if (FragManager.getInstance().getCurrentClass(baseActivity.getHomeDataOpe().getIndex()).getClass().getName().equals(CheckBloodFrag.class.getSimpleName())) {
            ...
            if (Pattern.compile(rule[i]).matcher(result).matches()) {
                switch (i) {
                    case 0:
                        ...
                        FragManager.getInstance().startFragment(activity.getSupportFragmentManager(), 0, new CheckBloodFrag());
                        break;
                    case 1:
                        break;
                    case 2:
    ...
    public void goToPatient(final FragmentActivity activity, final String pno) {
        ...
        if (fragment != null && fragment instanceof BedListFGM) {
            ...
            if (bedListFGM.getOpe().getDaOpe().getAllList() == null) {
                bedListFGM.getRegion2(new OnFinishListener() {
                    @Override
                    public void onFinish(Object o) {
                        Bundle bundle = new Bundle();
                        bundle.putSerializable(ValueConstant.DATA_DATA, bedListFGM.getOpe().getDaOpe().getAllList());
                        bundle.putSerializable(ValueConstant.DATA_DATA2, bedListFGM.getOpe().getDaOpe().getPatientBedResBean(bedListFGM.getOpe().getDaOpe().getAllList(), pno));
