...
public class AssaySortOpe {
    ...
    public ArrayList<AssayAdapterBean> sortAssay(ArrayList<AssayResBean> data){
        HashMap<String,ArrayList<AssayResBean>> hashMap=new HashMap<>();
        ArrayList<AssayAdapterBean> assayAdapterBeen  = new ArrayList<>();
        for(int i=0;i<data.size();i++){
            if(hashMap.get(data.get(i).getApplyno())==null){
                hashMap.put(data.get(i).getApplyno(),new ArrayList<AssayResBean>());
            }
            ArrayList<AssayResBean> l = hashMap.get(data.get(i).getApplyno());
            l.add(data.get(i));
        }
